CREATE DATABASE IF NOT EXISTS net_analysis;
USE net_analysis;
...
(
    ...
    `top_k_keys` JSON NOT NULL COMMENT "topk 的键",
...
CREATE DATABASE IF NOT EXISTS net_analysis_test;
USE net_analysis_test;
CREATE TABLE IF NOT EXISTS measurement_info
(
    `type` TINYINT NOT NULL COMMENT "类型，0-最小时间窗口, 1-分钟, 2-小时",
    `timestamp` LARGEINT NOT NULL COMMENT "秒级时间戳",
